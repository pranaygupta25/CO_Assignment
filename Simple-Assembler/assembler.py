...
temp = varLines
nextVariableLocation = len(assemblyCode) - varLines - blankLines
number = 1
while(varLines):
    ...
    if(line[0] == "var" and len(line) == 2):
        if(line[1] not in variables.keys()):
            ...
            encounteredErrors.append("ERROR at Line " + str(number) + ": Multiple declarations for the same variable found")
            number += 1
            continue
    elif(line[0] == "var"):
        encounteredErrors.append("ERROR at Line " + str(number) + ": Improper variable declaration")
        varLines -= 1
        assemblyCode.pop(0)
        number += 1
        continue
...
for lineNumber in range(len(assemblyCode)):
    ...
    if (assemblyCode[lineNumber].split()[0][-1] == ":"):
        if((assemblyCode[lineNumber].split()[0][:len(assemblyCode[lineNumber].split()[0]) - 1:]) not in labels.keys()):
            ...
            encounteredErrors.append("ERROR at Line " + str(lineNumber + varLines + 1) + ": Multiple declarations for the same label found")
            continue
...
for lineNumber in range(len(assemblyCode)):
    ...
    if (currentLine[0] == "var" and not variableDeclarationsNow):
        encounteredErrors.append("ERROR at Line " + str(lineNumber + varLines + 1) + ": Variable not declared at beginning")
        continue
    if (currentLine[0][-1] == ":"):
        encounteredErrors.append("ERROR at Line " + str(lineNumber + varLines + 1) + ": Use of multiple labels is not supported")
        continue


    # -------------------------------------------------------------------------------------------------------
    if (haltEncountered):
        encounteredErrors.append("ERROR at Line " + str(lineNumber + varLines + 1) + ": Halt(hlt) not being used as the last instruction")
        continue
    # _______________________________________________________________________________________________________

    # -------------------------------------------------------------------------------------------------------
    if (typeOfInstruction(currentLine[0]) == -1 and currentLine[0] != "mov"):
        encounteredErrors.append("ERROR at Line " + str(lineNumber + varLines + 1) + ": Invalid instruction")
        continue
    # _______________________________________________________________________________________________________
